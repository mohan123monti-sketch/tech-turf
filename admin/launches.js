    ...
    container.innerHTML = launches.map(launch => `
        <div class="bg-gray-800 rounded-xl border border-gray-700 p-6 relative overflow-hidden group">
            ...
            <div class="flex justify-between items-start mb-6 relative z-10">
                <div>
                    <span class="text-orange-500 font-mono text-xs tracking-widest uppercase mb-1 block">${launch.status}</span>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4 relative z-10">
                <div>
                    ...
                    <p class="text-sm text-gray-300 font-mono">${new Date(launch.date).toLocaleDateString()}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Rocket</p>
                    <p class="text-sm text-gray-300 font-mono">${launch.rocket}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Payload</p>
                    <p class="text-sm text-gray-300">${launch.payload}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Site</p>
                    <p class="text-sm text-gray-300">${launch.launchSite}</p>
                </div>
            </div>
...
function openLaunchModal(launchId = null) {
    ...
    if (launchId) {
        ...
        form.date.value = new Date(launch.date).toISOString().split('T')[0];
        form.rocket.value = launch.rocket;
        form.status.value = launch.status;
        form.payload.value = launch.payload;
        form.launchSite.value = launch.launchSite;
    } else {
...
async function saveLaunch(event) {
    ...
    const launchData = {
        ...
        rocket: form.rocket.value,
        status: form.status.value,
        payload: form.payload.value,
        launchSite: form.launchSite.value
    };
